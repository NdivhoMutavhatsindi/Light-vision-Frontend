...
function ComplianceModal({ request, onClose, onStatusUpdate }) {
  ...
  const handleUpdate = async () => {
    try {
      setSaving(true);
      await onStatusUpdate(request.compliance_request_id, status);
    } finally {
      setSaving(false);
    }
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50">
      ...
      >
        <div className="flex items-center justify-between mb-5">
          <h2 className="text-lg font-serif font-semibold text-navy-900">Compliance Request</h2>
...
export default function AgentCompliance() {
  ...
  const loadRequests = async () => {
    try {
      setLoading(true);
      const data = await getComplianceRequests();
      setRequests(data || []);
    } catch (error) {
      console.error(error);
    } finally {
  ...
  const handleStatusUpdate = async (id, status) => {
    try {
      await updateComplianceRequestStatus(id, status);
      await loadRequests();
      setSelected(null);
    } catch (error) {
      console.error(error);
            ...
            <table className="w-full">
              ...
              <tbody className="divide-y divide-gray-50">
                {loading ? (
                  ...
                  filtered.map((req) => (
                    <tr key={req.compliance_request_id} className="hover:bg-gray-50 transition-colors">
